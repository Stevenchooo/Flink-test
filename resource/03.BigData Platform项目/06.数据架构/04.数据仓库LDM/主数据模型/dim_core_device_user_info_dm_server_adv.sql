# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_server_adv.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: 广告用户设备信息
#  Describe: 广告用户全量设备信息（合法IMEI设备）
#  Input：  ODS_DEV_ADV_REQUEST_HM
...
PARTITION(pt_d='$date', pt_service='adv', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        (
            SELECT 
                if(FI_AesDecryptUDF(imei, 'a55e6886491df77f06f8379fd0a012bb0dc3cba84d1b5c74444c91af83c36778') is null,AesCBCOpenAlliance(imei,'pUvA7q/8jzzK+E8Ez/ZGRQ=='),FI_AesDecryptUDF(imei, 'a55e6886491df77f06f8379fd0a012bb0dc3cba84d1b5c74444c91af83c36778'))        AS imei,
                reqTime                                                                        AS create_time,
                IF(IsEmptyUDF(TerminalFormateUDF(model)), NULL, TerminalFormateUDF(model))     AS device_name,
                if(FI_AesDecryptUDF(mac, 'a55e6886491df77f06f8379fd0a012bb0dc3cba84d1b5c74444c91af83c36778') is null,AesCBCOpenAlliance(mac,'pUvA7q/8jzzK+E8Ez/ZGRQ=='),FI_AesDecryptUDF(mac, 'a55e6886491df77f06f8379fd0a012bb0dc3cba84d1b5c74444c91af83c36778'))        AS mac
            FROM ODS_DEV_ADV_REQUEST_HM
            WHERE pt_d = '$date'
        ) t
        WHERE IsDeviceIdLegal(imei) AND LENGTH(imei) < 17 AND to_date(create_time) IS NOT NULL
        UNION ALL
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'adv' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
