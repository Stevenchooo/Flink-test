# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_bdreporter.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: 大数据采集设备信息
#  Describe: 大数据采集全量设备信息（合法IMEI设备）
#  Input：  dw_emui_rom_info_dm
...
PARTITION(pt_d='$date', pt_service='bdreporter', pt_source = 'server')
SELECT
    t1.imei,
...
(
    ...
    (
        SELECT 
            imei                                                                          AS imei             ,
            NULL                                                                          AS did              ,
            IF(REGEXP(mac, '^[0-9a-zA-Z:]{17}$'), mac, NULL)                              AS mac              ,
            NULL                                                                          AS sn               ,
            IF(REGEXP(device_name, '^[0-9a-zA-Z \\\\-\\\\._\\\\+]+$'), device_name, NULL) AS device_name      ,
            NULL                                                                          AS device_color     ,
            NULL                                                                          AS ram_size         ,
            NULL                                                                          AS rom_size         ,
            eui_ver                                                                       AS first_eui_ver    ,
            eui_ver                                                                       AS last_eui_ver     ,
            rom_ver                                                                       AS first_rom_ver    ,
            rom_ver                                                                       AS last_rom_ver     ,
            NULL                                                                          AS android_ver      ,
            NULL                                                                          AS os_language      ,
            IF(REGEXP(plmn, '^\\\\d{5}$'), plmn, NULL)                                    AS first_plmn       ,
            IF(REGEXP(plmn, '^\\\\d{5}$'), plmn, NULL)                                    AS last_plmn        ,
            ip                                                                            AS first_ip         ,
            ip                                                                            AS last_ip          ,
        ...
        (
         ...
         WHERE to_date(create_time) IS NOT NULL
        UNION ALL
        SELECT 
            imei             ,
            did              ,
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'bdreporter' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
