# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_bisdk.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: BI SDK采集设备信息
#  Describe: BI SDK采集全量设备信息（合法IMEI设备）
#  Input：  t_appa_visit_dm
...
PARTITIONED BY (pt_d STRING,pt_service STRING)
...
PARTITION(pt_d='$date', pt_service='bisdk', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        (
            ...
            WHERE pt_d='$date' and IsDeviceIdLegal(device_id) AND LENGTH (device_id) < 17
        ) t111
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'bisdk' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
