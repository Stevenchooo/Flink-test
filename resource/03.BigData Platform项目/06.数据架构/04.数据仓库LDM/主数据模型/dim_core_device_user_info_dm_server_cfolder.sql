...
PARTITION(pt_d='$date', pt_service='cfolder', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        (
            SELECT 
                if(AesDecryptUDF2(deviceId) is null or trim(AesDecryptUDF2(deviceId)) = '',AesCBCOpenAlliance(deviceId,'DfbTEhKZnx+eA2ujf04+JQ=='),AesDecryptUDF2(deviceId)) AS imei,
                date_time          AS create_time
            FROM ods_dev_cloud_folder_app_view_dm 
            WHERE pt_d ='$date' AND to_date(date_time) IS NOT NULL
        ) t1111
        ...
        WHERE pt_d='$last_date' and pt_service='cfolder' and pt_source='server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
