...
PARTITION(pt_d='$date', pt_service='cloudplus', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        SELECT 
            ...
            NULL                                                                          AS app_ver          ,
            MIN(create_time)                                                              AS create_time      ,
            MAX(create_time)                                                              AS last_oper_time   ,
            NULL                                                                          AS huawei_flag
        FROM
        (
            SELECT revertDeviceId(aesudf(device_id_aes)) AS imei,
                time                                     AS create_time
            FROM ODS_CLOUD_PHONE_OPER_LOG_DM
            WHERE pt_d='$date' AND to_date(time) IS NOT NULL
        ) t11
        ...
        WHERE pt_d = '$last_date' 
            AND pt_service = 'cloudplus'
            AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
