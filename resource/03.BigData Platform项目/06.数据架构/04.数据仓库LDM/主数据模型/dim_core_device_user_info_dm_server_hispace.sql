...
PARTITION(pt_d='$date', pt_service='hispace', pt_source='server')
SELECT
    t1.imei,
    ...
    (
        ...
        (
            SELECT RevertDeviceId(SPLIT(logon_id,'@')[1])                                                          AS imei,
                MAX(IF(to_date(oper_time) is NULL OR oper_time > date_add('$date_ep', 1), '$date_ep' , oper_time)) AS create_time,
                NULL                                                                                               AS device_name,
            ...
            WHERE pt_d = '$date' AND IsDeviceIdLegal(SPLIT(logon_id,'@')[1]) AND LENGTH(SPLIT(logon_id,'@')[1]) < 17
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'hispace' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
