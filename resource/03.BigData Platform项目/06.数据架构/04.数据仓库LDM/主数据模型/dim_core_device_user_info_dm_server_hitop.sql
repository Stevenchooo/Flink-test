# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_server_hitop.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: 主题用户设备信息
#  Describe: 主题用户全量设备信息（合法IMEI设备）
#  Input：  t_appa_event_dm
...
PARTITION(pt_d='$date', pt_service='hitop', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        SELECT 
            RevertDeviceId(t113.device_id)                                                AS imei             ,
            NULL                                                                          AS did              ,
            NULL                                                                          AS mac              ,
            NULL                                                                          AS sn               ,
        ...
        (
            SELECT
                SPLIT(sign,'@')[1]                                                                      AS encrypted_device_id,
                IF(to_date(add_time) IS NULL, CONCAT('$date_ep', ' 00:00:00'), add_time)                AS create_time,
                IF(IsEmptyUDF(TerminalFormateUDF(phone_type)), NULL, TerminalFormateUDF(phone_type))    AS device_name,
                IF(REGEXP(CONCAT(mcc, mnc), '^\\\\d{5}$'), CONCAT(mcc, mnc), NULL)                      AS plmn,
                IF(REGEXP(firmware, '^[0-9\\\\.]+$'), firmware, NULL)                                   AS android_ver,
                IF(to_date(last_time) IS NULL, CONCAT('$date_ep', ' 00:00:00'), last_time)              AS last_oper_time
            FROM ODS_EUI_D_HITOP_REGISTER_DM
            WHERE pt_d = '$date'
        ) t112
        LEFT OUTER JOIN DIM_EUI_T_DEVICEID_SECRETKEY_DS t113
        ON t112.encrypted_device_id = t113.encrypted_device_id
        WHERE t113.device_id IS NOT NULL AND IsDeviceIdLegal(t113.device_id) AND LENGTH(t113.device_id) < 17
        UNION ALL
        ...
        SELECT 
            ...
            app_ver                                               AS app_ver,
            IF(to_date(create_time) IS NULL, '$last_date_ep', create_time)                              AS create_time,
            IF(to_date(last_oper_time) IS NULL, COALESCE(create_time, '$last_date_ep'), last_oper_time) AS last_oper_time,
            huawei_flag
        FROM dim_core_device_user_info_dm
        WHERE pt_d = '$last_date' AND pt_service = 'hitop' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
