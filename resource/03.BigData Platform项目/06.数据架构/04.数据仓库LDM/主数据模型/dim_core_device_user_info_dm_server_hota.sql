# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_server_hota.sql
...
PARTITION(pt_d='$date', pt_service='hota', pt_source='server')
SELECT
    t1.imei,
...
(
    SELECT
        ...
        MAX(IF(REGEXP(ram_size, '^[0-9GMBgbm]+$'), ram_size, NULL))                 AS ram_size,
        MAX(IF(REGEXP(rom_size, '^[0-9GMBgbm]+$'), rom_size, NULL))                 AS rom_size,
    ...
    (
        SELECT 
            ...
            NULL                                                                     AS mac              ,
            NULL                                                                     AS sn               ,
            IF(IsEmptyUDF(TerminalFormateUDF(device_name)) OR !REGEXP(TerminalFormateUDF(device_name), '^[0-9a-zA-Z \\\\-\\\\._\\\\+]+$'), NULL, TerminalFormateUDF(device_name)) AS device_name      ,
            SPLIT(saleinfo,'\\\\|')[0]                                               AS device_color     ,
            SPLIT(saleinfo,'\\\\|')[5]                                               AS ram_size         ,
            SPLIT(saleinfo,'\\\\|')[6]                                               AS rom_size         ,
            REGEXP_EXTRACT(SPLIT(saleinfo,'\\\\|')[4], '(\\\\d+\\\\.\\\\d+)', 1)     AS first_eui_ver    ,
            REGEXP_EXTRACT(SPLIT(saleinfo,'\\\\|')[4], '(\\\\d+\\\\.\\\\d+)', 1)     AS last_eui_ver     ,
            UPPER(firmware_version)                                                  AS first_rom_ver    ,
            UPPER(firmware_version)                                                  AS last_rom_ver     ,
            REGEXP_EXTRACT(client_os,'(\\\\d+\\\\.\\\\d+)',1)                        AS android_ver      ,
            os_language                                                              AS os_language      ,
            IF(REGEXP(SUBSTR(imsi,1,5), '^\\\\d{5}$'), SUBSTR(imsi,1,5), NULL)       AS first_plmn       ,
            IF(REGEXP(SUBSTR(imsi,1,5), '^\\\\d{5}$'), SUBSTR(imsi,1,5), NULL)       AS last_plmn        ,
            ...
            check_time                                                               AS last_oper_time   ,
            NULL                                                                     AS huawei_flag
        FROM ODS_HOTA_UPDATE_DEVICEINFO_DM
        WHERE pt_d = '$date' and product IN ('HID','SmartPhone') 
            AND IsDeviceIdLegal(imei) AND LENGTH(imei) < 17
            AND to_date(check_time) IS NOT NULL
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'hota' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
