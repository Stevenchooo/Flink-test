...
PARTITION(pt_d='$date', pt_service='installer', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        (
            SELECT 
                SUBSTR(deviceId_UDF(IF(IsEmptyUDF(imei) OR SIZE(SPLIT(imei, ':')) <= 1 OR LENGTH(SPLIT(imei, ':')[1]) < 32, imei,
                    CONCAT(SPLIT(imei, ':')[0],':', aesDecrypt(SPLIT(imei, ':')[1], 'hispace')))),6)                              AS imei,
                if(size(SPLIT(install_date, 'Date:'))<=1,null,SPLIT(install_date,'Date:')[1])                                     AS create_time
            FROM ods_hispace_apk_install_dm
            WHERE pt_d = '$date' 
                AND to_date(if(size(SPLIT(install_date, 'Date:'))<=1,null,SPLIT(install_date,'Date:')[1])) IS NOT NULL
        ) t11
        WHERE IsDeviceIdLegal(imei) AND LENGTH(imei) < 17
        UNION ALL
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'installer' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
