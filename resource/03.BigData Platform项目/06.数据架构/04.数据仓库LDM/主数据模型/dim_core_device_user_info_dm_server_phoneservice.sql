...
PARTITION(pt_d='$date', pt_service='phoneservice', pt_source='server')
SELECT
    t1.imei,
    ...
    IF(LENGTH(t1.create_time) = 14, dateUtil(t1.create_time, 'yyyyMMddHHmmss', 'yyyy-MM-dd HH:mm:ss'), t1.create_time) AS create_time,
    IF(LENGTH(t1.last_oper_time) = 14, dateUtil(t1.last_oper_time, 'yyyyMMddHHmmss', 'yyyy-MM-dd HH:mm:ss'), COALESCE(t1.last_oper_time, t1.create_time)) AS last_oper_time,
...
(
    ...
    (
        SELECT 
            COALESCE(t11.imei, t12.imei)                                                                     AS imei             ,
            NULL                                                                                             AS did              ,
            NULL                                                                                             AS mac              ,
            NULL                                                                                             AS sn               ,
            COALESCE(IF(REGEXP(t11.device_name, '^[0-9a-zA-Z \\\\-\\\\._\\\\+]+$'), t11.device_name, NULL),
                IF(REGEXP(t12.device_name, '^[0-9a-zA-Z \\\\-\\\\._\\\\+]+$'), t12.device_name, NULL))       AS device_name      ,
            NULL                                                                                             AS device_color     ,
            NULL                                                                                             AS ram_size         ,
            NULL                                                                                             AS rom_size         ,
            t12.eui_ver                                                                                      AS first_eui_ver    ,
            t12.eui_ver                                                                                      AS last_eui_ver     ,
            t12.rom_ver                                                                                      AS first_rom_ver    ,
            t12.rom_ver                                                                                      AS last_rom_ver     ,
            t12.android_ver                                                                                  AS android_ver      ,
            t12.os_language                                                                                  AS os_language      ,
            NULL                                                                                             AS first_plmn       ,
            NULL                                                                                             AS last_plmn        ,
            NULL                                                                                             AS first_ip         ,
            NULL                                                                                             AS last_ip          ,
            ...
            NULL                                                                                             AS last_area        ,
            NULL                                                                                             AS app_ver          ,
            COALESCE(t11.create_time, t12.create_time)                                                       AS create_time      ,
            COALESCE(t11.create_time, t12.create_time)                                                       AS last_oper_time   ,
            NULL                                                                                             AS huawei_flag
        FROM
        (
            SELECT RevertDeviceId(deviceid)                                                                   AS imei,
                MAX(memberbindtime)                                                                           AS create_time,
                MAX(IF(IsEmptyUDF(TerminalFormateUDF(terminaltype)), NULL, TerminalFormateUDF(terminaltype))) AS device_name
            FROM ods_up_t_up_memberright_dm_cypt
            WHERE pt_d = '$date' AND IsDeviceIdLegal(deviceid) AND LENGTH(deviceid) < 17
                AND to_date(memberbindtime) IS NOT NULL
            GROUP BY RevertDeviceId(deviceid)
        )t11
        FULL OUTER JOIN
        (
            SELECT RevertDeviceId(CommonAesCBCDecryptUDF(aes_imei, 'qstn'))                               AS imei,
                MAX(dateUtil(time, 'yyyyMMddHHmmss', 'yyyy-MM-dd HH:mm:ss'))                              AS create_time,
                MAX(IF(IsEmptyUDF(TerminalFormateUDF(devicetype)), NULL, TerminalFormateUDF(devicetype))) AS device_name,
                MAX(REGEXP_EXTRACT(SUBSTR(os,9), '(\\\\d+\\\\.\\\\d+)', 1))                               AS android_ver,
                MAX(UPPER(firmware))                                                                      AS rom_ver,
                MAX(language)                                                                             AS os_language,
                MAX(REGEXP_EXTRACT(emuiversion, '(\\\\d+\\\\.\\\\d+)', 1))                                AS eui_ver
            FROM ods_phoneservice_qstn_receivesurvey_log_dm
            WHERE pt_d = '$date' 
                AND IsDeviceIdLegal(CommonAesCBCDecryptUDF(aes_imei, 'qstn')) 
                AND LENGTH(CommonAesCBCDecryptUDF(aes_imei, 'qstn')) < 17
                AND to_date(dateUtil(time, 'yyyyMMddHHmmss', 'yyyy-MM-dd HH:mm:ss')) IS NOT NULL
            GROUP BY RevertDeviceId(CommonAesCBCDecryptUDF(aes_imei, 'qstn'))
        ) t12
        ON t11.imei = t12.imei
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'phoneservice' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
