# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_server_push.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: push用户设备信息
#  Describe: push用户全量设备信息（合法IMEI设备）
#  Input：  dw_push_imei_info_dm,
...
PARTITION (pt_d='$date',pt_service='push',pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        (
            SELECT 
                RevertDeviceId(dev_id)                                                       AS imei,
                NULL                                                                         As device_name,
                NULL                                                                         As plmn,
                '$date_ep'                                                                   AS create_time
            FROM ods_push_dev_login_new_dm 
            WHERE pt_d = '$date'  AND size(info) > 1 
                AND IsDeviceIdLegal(RevertDeviceId(dev_id)) 
                AND LENGTH(RevertDeviceId(dev_id)) < 17
            UNION ALL 
            SELECT RevertDeviceId(imei)                                                  AS imei,
                IF(IsEmptyUDF(TerminalFormateUDF(mode)), NULL, TerminalFormateUDF(mode)) AS device_name,
                IF(REGEXP(CONCAT(mcc, mnc), '^\\\\d{5}$'), CONCAT(mcc, mnc), NULL)       AS plmn,
                record_time                                                              AS create_time
            FROM ods_push_routerecord_dm 
            WHERE pt_d = '$date' AND to_date(record_time) = '$date_ep' 
                AND IsDeviceIdLegal(RevertDeviceId(imei)) 
                AND LENGTH(RevertDeviceId(imei)) < 17
        ) t11
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'push' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
