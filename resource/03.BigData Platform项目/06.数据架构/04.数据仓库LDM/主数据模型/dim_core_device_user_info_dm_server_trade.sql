# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_server_trade.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: 支付用户设备信息
#  Describe: 支付全量设备信息（合法IMEI设备）
...
PARTITION(pt_d='$date', pt_service='trade', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        FROM
        (
            SELECT RevertDeviceId(device_id)                                                            AS imei,
                IF(to_date(trade_time) IS NULL, '$date_ep', trade_time)                                 AS create_time,
                IF(IsEmptyUDF(TerminalFormateUDF(device_type)), NULL, TerminalFormateUDF(device_type))  AS device_name
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'trade' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
