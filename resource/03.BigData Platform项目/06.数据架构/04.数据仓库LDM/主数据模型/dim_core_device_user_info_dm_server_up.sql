...
PARTITION(pt_d='$date', pt_service='up', pt_source='server')
SELECT
    t1.imei,
...
(
    SELECT
        ...
        MIN(CONCAT(create_time, '\001', first_province, '\001', first_city))          AS first_area,
        MAX(CONCAT(last_oper_time, '\001', resident_province, '\001', resident_city)) AS resident_area,
        MAX(CONCAT(last_oper_time, '\001', last_province, '\001', last_city))         AS last_area,
        SPLIT(MAX(CONCAT(last_oper_time, '\001', app_ver)), '\001')[1]                AS app_ver,
        MIN(create_time)                                                              AS create_time,
        MAX(last_oper_time)                                                           AS last_oper_time,
        MAX(huawei_flag)                                                              AS huawei_flag
    FROM 
    (
    SELECT 
          imei,             
    ...
    WHERE pt_d='$date' and pt_source='up'
    ...
    SELECT 
            ...
            NULL                                    AS mac              ,
            NULL                                    AS sn               ,
            IF(!REGEXP(TerminalFormateUDF(terminal_type), '^[0-9a-zA-Z \\\\-\\\\._\\\\+]+$'), NULL, 
       TerminalFormateUDF(terminal_type))           AS    device_name   ,
            ...
            NULL                                    AS first_ip         ,
            NULL                                    AS last_ip          ,
            NULL                                    AS first_province,   
            NULL                                    AS first_city,       
            NULL                                    AS resident_province,
            NULL                                    AS resident_city,    
            NULL                                    AS last_province,    
            NULL                                    AS last_city,        
            NULL                                    AS app_ver          ,
            terminal_settime                        AS create_time      ,
            terminal_settime                        AS last_oper_time   ,
            NULL                                    AS huawei_flag
    FROM    ODS_UP_USER_DEVICE_INFO_DM
    WHERE   PT_D='$date'
    ) tt0
    GROUP BY imei
) t1
