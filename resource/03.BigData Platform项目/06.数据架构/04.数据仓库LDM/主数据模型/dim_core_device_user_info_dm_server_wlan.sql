...
PARTITION(pt_d='$date', pt_service='wlan', pt_source='server')
SELECT
    t1.imei,
...
(
    ...
    (
        ...
        (
            SELECT
                DecryptAES(imei_crypt)                                                                 AS imei,
                IF(IsEmptyUDF(TerminalFormateUDF(device_name)), NULL, TerminalFormateUDF(device_name)) AS device_name,
                create_time                                                                            AS create_time
            FROM ODS_VSIM_CHG_ACCOUNT_DM
            WHERE pt_d ='$date' AND to_date(create_time) IS NOT NULL
        ) t11
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'wlan' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
