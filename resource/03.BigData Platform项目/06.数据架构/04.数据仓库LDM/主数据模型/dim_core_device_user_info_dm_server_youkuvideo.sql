# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_server_youkuvideo.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: 优酷视频用户设备信息
#  Describe: 优酷视频用户全量设备信息（合法IMEI设备）
#  Input：  ODS_HWVIDEO_INTERFACE_DM
...
PARTITION(pt_d='$date', pt_service='youkuvideo', pt_source='server')
SELECT
    t1.imei,
    ...
    (
        ...
        (
            SELECT
                ...
                logtime                                                                                 AS logtime,
                version                                                                                 AS version
            FROM ODS_HWVIDEO_INTERFACE_DM
            WHERE pt_d = '$date' AND IsDeviceIdLegal(TRIM(imei)) AND LENGTH(TRIM(imei)) < 17
                AND to_date(logtime) IS NOT NULL
        ) t11
        ...
        WHERE pt_d = '$last_date' AND pt_service = 'youkuvideo' AND pt_source = 'server'
    ) t11
    GROUP BY imei
) t1
LEFT OUTER JOIN
