# ----------------------------------------------------------------------------
#  File Name:   dim_core_device_user_info_dm_up_service.sql
#  Copyright(C)Huawei Technologies Co.,Ltd.1998-2016.All rights reserved.
#  Purpose: 各业务使用UP信息
#  Describe: 各业务使用UP信息（合法IMEI设备）插入到业务分区的up渠道
...
(
    ...
    (
        ...
        FROM
        (
            SELECT t1.imei                                                                AS imei,
                MIN(t1.oper_time)                                                         AS create_time,
                MAX(t1.oper_time)                                                         AS last_oper_time,
                SPLIT(MIN(CONCAT(t1.oper_time,'#',t1.device_name)),'#')[1]                AS device_name,
				SPLIT(MIN(CONCAT(t1.oper_time,'#',t1.oper_ip)),'#')[1]                    AS first_ip,
				SPLIT(MAX(CONCAT(t1.oper_time,'#',t1.oper_ip)),'#')[1]                    AS last_ip,
                COALESCE(t2.service_id,'other')                                           AS pt_service
            FROM
            ( 
                SELECT     imei,
                    device_name,
                ...
                WHERE pt_d = '$date' AND IsDeviceIdLegal(imei) AND LENGTH(imei) < 17 AND to_date(oper_time) IS NOT NULL
            ) t1  
            LEFT OUTER JOIN 
              dim_core_pkg2service2up_ds t2 on t1.pt_service=t2.service_id
            GROUP BY t1.imei,COALESCE(t2.service_id,'other')
        ) t1
        UNION ALL
        ...
        WHERE pt_d = '$last_date'  AND pt_source = 'up'
    ) t11
    GROUP BY imei,pt_service
) t1
LEFT OUTER JOIN
(
    SELECT 
