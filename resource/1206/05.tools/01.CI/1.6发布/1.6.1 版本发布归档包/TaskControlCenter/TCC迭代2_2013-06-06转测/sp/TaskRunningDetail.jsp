...
<script>
		...
		$.ajax({
			...
			success : function(data, textStatus) {
				if (textStatus == "success") {
					...
					var returnValue2PageType = jsonObj["returnValue2PageType"];
					var values = jsonObj["values"];
					var noPriTasks = jsonObj.extValue;
					if (success == true) {
						if(3 == returnValue2PageType)
						{
							if(values.length >= 1)
							{
								...
								//绑定
								loadDependRSListLocal(taskId,cycleId,dependRSJsonObj,noPriTasks);
							}
						}
					} 
					else
	...
	//本地加载依赖的任务周期运行状态信息列表
	function loadDependRSListLocal(taskId,cycleId,jsonObj,noPriTasks)
	{
		if(null != noPriTasks && noPriTasks.length > 0)
		{
			showTPTips("<font color='red'>没有如下任务的查看权限，已经隐藏相应任务周期的展示!</font>",noPriTasks);
		}
	...
	function loadJobsList(taskId,cycleId)
	{
		$('#jobIdList').datagrid({
			...
			singleSelect:true,
			collapsible : true,
			url : 'jobIds2Json?taskId='+taskId+'&cycleId='+cycleId,
			sortName : '任务Id',
			sortOrder : 'taskId',
			remoteSort : false,
			idField : 'taskId',
