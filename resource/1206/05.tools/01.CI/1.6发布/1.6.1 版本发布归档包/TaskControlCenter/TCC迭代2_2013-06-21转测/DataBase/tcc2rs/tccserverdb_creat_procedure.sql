...
begin
   ...
   set msql=concat('select ''21'' as db_id,f.*,date(start_time) as run_date,unix_timestamp(f.end_time) - unix_timestamp(f.start_time) as execute_seconds from
...
into outfile  ''/home/mysql/tasks/task_instance_',curr_time_str,'.info_21''');
...
DROP EVENT IF EXISTS `crontab_export_tasks`;
DELIMITER ;;

CREATE EVENT `crontab_export_tasks` ON SCHEDULE EVERY 1 DAY STARTS '2013-06-09 08:00:00' ON COMPLETION PRESERVE ENABLE DO call export_task_running_info();;
