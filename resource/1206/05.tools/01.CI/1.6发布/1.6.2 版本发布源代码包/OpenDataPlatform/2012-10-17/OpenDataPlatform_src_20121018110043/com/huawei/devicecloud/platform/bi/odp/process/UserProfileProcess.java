...
public class UserProfileProcess
{
        ...
        throws CException
    {
        ...
        if (null != rservedInfo)
        {
            if (rservedInfo.getState().equals(ReservedStateType.FINISH))
            {
                ...
                {
                    String mondayStr = OdpCommonUtils.getMondayDateStr(rservedInfo.getUseDate());
                    //创建crt表
                    String crt = String.format("crt_%s", mondayStr);
                    //删除crt表中预留Id相关记录
                    deleteCRT(crt, reserveId);
                    //删除数据结果表
                    dropResultTable(rservedInfo.getResultTableName());
                    //删除数据文件
    ...
    //删除crt表中的预留Id相关记录
    private void deleteCRT(String crt, String reserveId)
        throws CException
    {
        if (StringUtils.isEmpty(crt) || StringUtils.isEmpty(reserveId))
        {
            return;
        }
        
        odpDao.deleteCRT(crt, reserveId);
    }
    
    /**
     * 回收资源
     * @param reservedInfo 预留信息
     * @throws CException 异常
     */
    public void reclaimResource(ReservedInfoEntity reservedInfo)
        throws CException
    {
        //为空直接退出
        if (null != reservedInfo)
        {
            String mondayStr = OdpCommonUtils.getMondayDateStr(reservedInfo.getUseDate());
            //创建crt表
            String crt = String.format("crt_%s", mondayStr);
            //删除对应的crt表（因为是延迟下周2凌晨删除，所以预留记录过期时就可以删除周表）
            odpDao.dropCRT(crt);
            //删除数据结果表
            dropResultTable(reservedInfo.getResultTableName());
            //删除数据文件
            OdpCommonUtils.deleteFile(reservedInfo.getFileUrl());
            //删除预留信息
            deleteReservedInfo(reservedInfo.getReserveId());
        }
    }
        ...
        throws CException
    {
        ...
        try
        {
            ...
            {
                //创建读取文件
                List<UserProfileEntity> records = getUPRecords(rservedInfo.getResultTableName());
                //获取列名数组
    ...
    {
        return OdpCommonUtils.getChoosedCFTMappings(this.odpDao, resultMapId, columnNames);
    }
    ...
    /**
     * 删除结果表
     * @param resTableName 结果表名
     * @throws CException 异常
     */
    public void dropResultTable(String resTableName)
        throws CException
    {
        if (StringUtils.isEmpty(resTableName))
        {
            return;
        }
        
        odpDao.dropTempTable(resTableName);
    }
     ...
     * @return crts表的记录总数
     */
    public Long getSum(List<String> crts)
    {
        Long sum = 0L;
        for (String crt : crts)
        {
            sum += odpDao.getCRTCount(crt);
        }
        return sum;
    }
}
