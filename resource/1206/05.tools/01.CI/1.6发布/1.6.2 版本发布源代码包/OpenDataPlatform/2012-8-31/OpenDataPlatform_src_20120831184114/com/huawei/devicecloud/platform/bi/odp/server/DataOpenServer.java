...
public class DataOpenServer
{
    ...
    //用户信息表表名字
    private String tableName = "user_profile_table";
    ...
    //新增预留信息，获取预留Id
    private String createReservedInfo(String appId)
        throws CException
    {
        ...
        throws CException
    {
        ...
        try
        {
            ...
            //拼接sql并顺便检查列是否存在
            String selectSql =
                constructSelectSql(req.getExtractList(),
                    tableName,
                    controlFlag.getCurrentTableID(),
                    req.getFilterStmt(),
                    maxRecordCount);
     ...
     * @return 查询Sql
     */
    private String constructSelectSql(List<Integer> columns, String dataTableName, Integer currentTableID,
        String filterStmt, Long limit)
        throws CException
    {
        ...
        selectSqlSb.append(String.format("%s_%d", dataTableName, currentTableID));
        
        //过滤条件不为空才拼接where
        if (!StringUtils.isBlank(filterStmt))
        {
            selectSqlSb.append(" where ");
            selectSqlSb.append(filterStmt);
        }
     ...
     */
    class AsyncQueryDataCount extends ParallelCountor implements Runnable
    {
        //查询记录总数请求提
     ...
     * @version [Open Data Platform Service, 2012-8-9]
     */
    class AsyncReserveBatchData implements Runnable
    {
        //查询sql语句
        ...
        public void run()
        {
            ...
            try
            {
                ...
                //预留信息数组
                ReservedInfo[] reservedInfos = getReservedInfo(rbdReq.getAppId(), rbdReq.getBatchInfo());
                
                //获取表字段名集合
                ColumnFieldTypeMapping[] cftMapping = getColumnFieldTypeMappings(rbdReq.getExtractList(), resultMapID);
                
                //创建临时表
                String[] tempTableName = createTempTables(reservedInfos, cftMapping);
                ...
                long[] recordNums = processRecords(cftMapping, recordQueue, tempTableName, rfileNames);
                ...
                //更新临时表预留数据文件名
                saveReservedInfo(reservedInfos, columnNames, tempTableName, rfileNames);
        ...
        /**
         * 保存预留信息数组、列名字，临时表名数组，文件名数组
         */
        private void saveReservedInfo(ReservedInfo[] reservedInfos, String columnNames, String[] tempTableName,
            String[] rfileNames)
            throws CException
        {
            //更新失效时间
            ...
            for (int i = 0; i < reservedNum; i++)
            {
                ...
                if (writeRecords)
                {
                    reservedE.setTmpTableName(tempTableName[i]);
        ...
        private long[] processRecords(ColumnFieldMapping[] cfMappings, RecordQueue<UserProfileEntity> recordQueue,
            String[] tempTableName, String[] rfileNames)
            throws CException
        {
            //获取比例
            ...
            TempTableRecordProcess<UserProfileEntity> tableProcess =
                new TempTableRecordProcess<UserProfileEntity>(cfMappings, tempTableName, rfileNames, ratios,
                    recordQueue);
            //传递dao对象
        ...
        //获取预留信息
        private ReservedInfo[] getReservedInfo(String appId, List<DateRatioInfo> batchs)
            throws CException
        {
            //预留信息数组
         ...
         * 创建临时表
         */
        private String[] createTempTables(ReservedInfo[] reservedInfos, ColumnFieldMapping[] columnFields)
            throws CException
        {
