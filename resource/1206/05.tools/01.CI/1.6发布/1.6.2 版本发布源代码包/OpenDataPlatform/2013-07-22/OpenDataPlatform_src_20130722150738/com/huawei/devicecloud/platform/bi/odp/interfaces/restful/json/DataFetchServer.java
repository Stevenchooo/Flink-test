...
public class DataFetchServer
{
    ...
    public ModelAndView wgetFileReq(HttpServletResponse response, @RequestBody
    String body, @RequestHeader(value = "Authorization", required = false)
    String authenInfo)
    {
        LOGGER.info("Enter wgetFileReq,request body is {}", new Object[] {Encoder.getInstance().truncatEncode(body)});
        ...
        try
        {
            ...
            LOGGER.info("Exit wgetFileReq successfully,request body is {}, return body is {}", new Object[] {body, rsp});
            return null;
        }
        catch (Exception e)
        {
        ...
        //执行时间统计
        StatFilter.timeStat(String.format("[group_fetch(ts=%s)]", null == req ? "" : req.getTimestamp()));
        //设置错误状态码
