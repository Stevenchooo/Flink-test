...
public class DataQueryClient
{
        ...
        throws CException
    {
        ...
        {
            LOGGER.error("callBackUrl is blank! req is {}", Encoder.getInstance().truncatEncode(req.toString()));
        ...
        LOGGER.info("call queryDataCountResp start, request is {}", Encoder.getInstance().truncatEncode(body));
        ...
        try
        {
            ...
            LOGGER.info("call queryDataCountResp end, response is {}",
                Encoder.getInstance().truncatEncode(httprsp.toString()));
            if (null != httprsp && HttpRequester.HTTP_OK == httprsp.getCode())
            {
                ...
                {
                    LOGGER.error("call queryDataCountResp failed! httprsp is {}",
                        Encoder.getInstance().truncatEncode(httprsp.toString()));
                    throw new CException(ResultCode.SYSTEM_ERROR);
                }
            }
            else
            {
                ...
                LOGGER.error("not correct responce for called queryDataCountResp! request is {},response is {}",
                    new Object[] {Encoder.getInstance().truncatEncode(req.toString()),
                        Encoder.getInstance().truncatEncode(httprsp.toString())});
                throw new CException(ResultCode.SYSTEM_ERROR);
            }
        }
        ...
        throws CException
    {
        ...
        //HttpPostRequest httpreq = buildHttpPostReq(body, callBackUrl);
        LOGGER.info("call reserveBatchDataResp start, request is {}", Encoder.getInstance().truncatEncode(body));
        //HttpPostResponse httprsp = remoteHttpService.sendHttpPostMessage(httpreq);
        HttpResponse httprsp = null;
        try
        {
            ...
            LOGGER.info("call reserveBatchDataResp end, response is {}",
                Encoder.getInstance().truncatEncode(httprsp.toString()));
            if (null != httprsp && HttpRequester.HTTP_OK == httprsp.getCode())
            {
                //获取http响应消息
                ...
                {
                    //记录错误抛出异常
                    LOGGER.error("call reserveBatchDataResp failed! httprsp is {}", Encoder.getInstance()
                        .truncatEncode(httprsp.toString()));
                    throw new CException(ResultCode.SYSTEM_ERROR);
                }
            }
            else
