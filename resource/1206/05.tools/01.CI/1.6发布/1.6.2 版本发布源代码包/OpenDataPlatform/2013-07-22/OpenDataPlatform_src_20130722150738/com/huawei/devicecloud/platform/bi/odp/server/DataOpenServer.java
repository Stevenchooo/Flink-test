...
import com.huawei.devicecloud.platform.bi.odp.constants.TransFlag;
...
import com.huawei.devicecloud.platform.bi.odp.entity.ReservedNewInfoEntity;
...
import com.huawei.devicecloud.platform.bi.odp.jersey.req.GroupFetchReq;
import com.huawei.devicecloud.platform.bi.odp.jersey.req.GroupRevokeReq;
import com.huawei.devicecloud.platform.bi.odp.jersey.req.GroupsPrepareReq;
import com.huawei.devicecloud.platform.bi.odp.jersey.rsp.GroupRevokeRsp;
import com.huawei.devicecloud.platform.bi.odp.jersey.rsp.GroupsPrepareRsp;
import com.huawei.devicecloud.platform.bi.odp.jersey.rsp.UploadFileRsp;
...
//            }
    ...
    private void cleanReservedInfo()
        throws CException
    {
        //获取过期的groupid对应的信息列表
        List<ReservedNewInfoEntity> list = this.odpDao.getExpiredReservedNewInfoList();
        
        //遍历列表，删除文件
        for (ReservedNewInfoEntity entity : list)
        {
            //更新状态
            entity.setState(ReservedStateType.EXPIRED);
            this.odpDao.updateReservedNewInfo(entity);
...
//        dealFileUploadTable(fileIdList);
    ...
    private void dealFileAndPath(List<String> fileIdList)
    {
        if (null != fileIdList && !fileIdList.isEmpty())
        {
            for (String fileId : fileIdList)
            {
                try
                {
                    String tempId = fileId.substring(fileId.lastIndexOf("/") + 1);
                    //获取文件名 与 文件夹名
                    String filePath = CommonUtils.getSysConfig("file_local_path");
                    
                    String fileName = filePath + File.separator + tempId + ".zip";
                    
                    String fileDir = filePath + File.separator + tempId;
        ...
        throws CException
    {
        ...
        int transFlag = Integer.parseInt(CommonUtils.getSysConfig("interface.switch"));
        if (TransFlag.YES == transFlag)
        {
            return wgetFileNew(req);
        }
        else
        {
            return wgetFileOld(req);
        }
     ...
     * @return 获取文件对象
     */
    private File wgetFileNew(WGetFileReq getFileReq)
        throws CException
    {
        //获取文件对象
        String reserveId = getFileReq.getReserveId();
        
        ReservedNewInfoEntity entity = this.odpDao.getReservedNewInfo(reserveId);
        ...
        //没有数据文件
        if (ReservedStateType.INIT == entity.getState())
        {
            
            return getFileByInterface(reserveId);
        }
        else
        {
            return new File(entity.getFileUrl());
        }
        
    }
    
    private File getFileByInterface(String groupId)
        throws CException
    {
        String filepath = OdpConfig.createOdpConfig().getRfilesDir() + System.currentTimeMillis();
        File file = new File(filepath);
        //调用bt_servser接口获取文件
        String url =
            "http://" + CommonUtils.getSysConfigValueByKey("bt_servser.opendataplatform.addr")
                + "/rest/dataplatform/groupFetch";
        
        Client client = Client.create();
        WebResource resource = client.resource(url);
        
        GroupFetchReq req = new GroupFetchReq();
        GroupFetchInfo info = new GroupFetchInfo();
        info.setGroupId(groupId);
        req.setGroupFetchInfo(info);
        String authInfo = CommonUtils.getInterFaceAuthInfo(info.toString());
        req.setAuthInfo(authInfo);
        String request = JSON.toJSONString(req);
        
        LOGGER.debug("transfer to groupFetch begin! req is {}", request);
        byte[] rsp = resource.type(MediaType.APPLICATION_JSON).post(byte[].class, request);
        LOGGER.debug("transfer to groupFetch end!");
        
        if (rsp.length == 0)
        {
            LOGGER.error("transfer groupFetch error! file is not exit");
            
            //生成文件失败，返回上传文件失败异常
            throw new CException(ResultCode.GROUPS_FETCH_INTERFACE_ERROR,
                "transfer to /rest/dataplatform/groupFetch interface error!");
        }
        
        InputStream input = new ByteArrayInputStream(rsp);
        
        try
        {
//            OdpFileUtils.compressFile(file, input);
            //写文件
            FileUtils.copyInputStreamToFile(input, file);
        }
        catch (Exception e)
        {
            LOGGER.error("write file error ! Exception is {}", e);
            throw new CException(ResultCode.WRITE_FILE_ERROR, "compress file  error!");
        }
        
        //更新数据库状态
        ReservedNewInfoEntity entity = new ReservedNewInfoEntity();
        entity.setGroupId(groupId);
        entity.setState(ReservedStateType.FINISH);
     ...
     * @return 获取文件对象
     */
    private File wgetFileOld(WGetFileReq req)
        throws CException
    {
        //获取文件对象
        ...
        throws CException
    {
        ...
        if (TransFlag.YES == transFlag)
        {
            revokeReserveNew(req);
        }
        else
        {
            revokeReserveOld(req);
        }
        
    }
    
    private void revokeReserveNew(RevokeReserveReq req)
        throws CException
    {
        
        String groupId = req.getReserveId();
        
        //查询此id是否合法
        ReservedNewInfoEntity entity = this.odpDao.getReservedNewInfo(groupId);
        
        //包含此预留信息，并且数据没有被取出可以取消预留
        if (null != entity && ReservedStateType.INIT == entity.getState())
        {
            // 调用接口取消用户预留
            String url =
                "http://" + CommonUtils.getSysConfigValueByKey("bt_servser.opendataplatform.addr")
                    + "/rest/dataplatform/groupRevoke";
            
            Client client = Client.create();
            WebResource resource = client.resource(url);
            
            GroupRevokeReq groupRevokeReq = new GroupRevokeReq();
            GroupFetchInfo info = new GroupFetchInfo();
            info.setGroupId(groupId);
            groupRevokeReq.setGroupFetchInfo(info);
            String authInfo = CommonUtils.getInterFaceAuthInfo(info.toString());
            groupRevokeReq.setAuthInfo(authInfo);
            String request = JSON.toJSONString(groupRevokeReq);
            
            LOGGER.debug("transfer to groupRevoke begin! req is {}", request);
            String result = resource.type(MediaType.APPLICATION_JSON).post(String.class, request);
            LOGGER.debug("transfer to groupRevoke end! rsp is {}", result);
            
            GroupRevokeRsp rsp = OdpCommonUtils.parseObject(result, GroupRevokeRsp.class).createGroupRevokeRsp();
            
            if (rsp.getResultCode() != ResultCode.SUCCESS)
            {
                LOGGER.error("transfer groupRevoke error! GroupRevokeRsp is {}", rsp);
                
                //生成文件失败，返回上传文件失败异常
                throw new CException(ResultCode.GROUPS_REVOKE_INTERFACE_ERROR,
                    "transfer to /rest/dataplatform/groupRevoke interface error!");
            }
            
            //删除此预留信息
            this.odpDao.deleteNewReservedInfo(groupId);
        }
        
    }
    
    private void revokeReserveOld(RevokeReserveReq req)
        throws CException
    {
        //获取预留信息
        ...
        throws CException
    {
        ...
        if (TransFlag.YES == transFlag)
        {
            reserveBatchDataInFastBit(req);
        }
        else
        {
            reserveBatchDataInMysqlDB(req);
        }
     ...
     * @throws CException 异常
     */
    private void reserveBatchDataInFastBit(ReserveBatchDataReq reserveBatchDataReq)
        throws CException
    {
        String url =
            "http://" + CommonUtils.getSysConfigValueByKey("bt_servser.opendataplatform.addr")
                + "/rest/dataplatform/groupsPrepare";
        
        Client client = Client.create();
        WebResource resource = client.resource(url);
        GroupsPrepareReq req = new GroupsPrepareReq();
        GroupsPrepareInfo info = new GroupsPrepareInfo();
        info.setDays(reserveBatchDataReq.getDays());
        String filterStmt = reserveBatchDataReq.getFilterStmt();
        int userAdFlag = reserveBatchDataReq.getUserAdFlag();
        //普通用户
        if (UserAdFlagType.NORMAL == userAdFlag)
        {
            info.setFilterStmt(filterStmt + " and user_ad_flag = '0'");
//            info.setFilterStmt(filterStmt);
            info.setLimitFlag(true);
        }
        //白名单用户
        else
        {
            info.setFilterStmt(filterStmt + " and user_ad_flag = '1'");
//            info.setFilterStmt(filterStmt);
            info.setLimitFlag(false);
        }
        
        info.setImeiFileId(reserveBatchDataReq.getImeiFileId());
        info.setRecordCount(reserveBatchDataReq.getRecordNumber());
        info.setBatchInfo(reserveBatchDataReq.getBatchInfo());
        String authinfo = CommonUtils.getInterFaceAuthInfo(info.toString());
        req.setAuthInfo(authinfo);
        req.setGroupsPrepareInfo(info);
        String request = JSON.toJSONString(req);
        LOGGER.debug("transfer to groupsPrepare begin! req is {}", request);
        String result = resource.type(MediaType.APPLICATION_JSON).post(String.class, request);
        LOGGER.debug("transfer to groupsPrepare end! rsp is {}", result);
        
        GroupsPrepareRsp rsp = OdpCommonUtils.parseObject(result, GroupsPrepareRsp.class).createGroupsPrepareRsp();
        if (rsp.getResultCode() != 0)
        {
            LOGGER.error("transfer groupsPrepare error! GroupsPrepareRsp is {}", rsp);
            
            //生成文件失败，返回上传文件失败异常
            throw new CException(ResultCode.GROUPS_PREPARE_INTERFACE_ERROR,
                "transfer to /rest/dataplatform/groupsPrepare interface error!");
        }
        
        //异步处理回调接口
        AsyncInterFaceReserveBatchData asyncExector =
            new AsyncInterFaceReserveBatchData(reserveBatchDataReq, rsp, this.odpDao);
     ...
     * @throws CException 异常
     */
    private void reserveBatchDataInMysqlDB(ReserveBatchDataReq req)
        throws CException
    {
        //isNeedLoadCtrl();
     ...
     * @see  [相关类/方法]
     */
    private class AsyncInterFaceReserveBatchData implements Runnable
    {
        //数据预留请求
        private ReserveBatchDataReq reserveBatchDataReq;
        
        //调用群体查询接口返回
        private GroupsPrepareRsp groupsPrepareRsp;
        ...
        /**
         * <默认构造函数>
         * @param reserveBatchDataReq
         * @param groupsPrepareRsp
         * @param odpDao 
         */
        AsyncInterFaceReserveBatchData(ReserveBatchDataReq reserveBatchDataReq, GroupsPrepareRsp groupsPrepareRsp,
            OdpDao odpDao)
        {
            this.reserveBatchDataReq = reserveBatchDataReq;
            this.groupsPrepareRsp = groupsPrepareRsp;
            this.odpDao = odpDao;
        }
        ...
        public void run()
        {
            ...
            req.setGroup_info(groupsPrepareRsp.getGroupInfo());
            req.setResult_code(groupsPrepareRsp.getResultCode());
            req.setTransaction_id(reserveBatchDataReq.getTransactionId());
            //交给稳定的服务发送，当前直接发送
            try
            {
                dataQueryClient.reserveBatchDataResp(reserveBatchDataReq.getCallbackURL(), req);
                
            }
            catch (Exception e)
            {
                LOGGER.error("dataQueryClient.reserveBatchDataResp failed! callbackURL is {}, req is {}.",
                    reserveBatchDataReq.getCallbackURL(),
                    req);
            }
            ...
            {
                //先新的数据表插入数据
                String appId = reserveBatchDataReq.getAppId();
                int days = reserveBatchDataReq.getDays();
                List<GroupInfo> groupInfoList = groupsPrepareRsp.getGroupInfo();
                
                //只有预留用户，预留的group_id才是有效的，否则无效，不需要记录至数据库
                if (days > 0)
                {
                    for (GroupInfo groupInfo : groupInfoList)
                    {
                        Date date = CommonUtils.convertDateTimeFormat(groupInfo.getDate(), null);
                        Date expireDate = OdpCommonUtils.getNextMondayDateStr(date);
                        String groupId = groupInfo.getGroup_id();
                        long count = groupInfo.getRecord_count();
                        ReservedNewInfoEntity entity = new ReservedNewInfoEntity();
                        entity.setGroupId(groupId);
                        entity.setDays(days);
                        entity.setAppId(appId);
                        entity.setUseDate(date);
                        entity.setExpiredDate(expireDate);
                        entity.setState(ReservedStateType.INIT);
                        entity.setCnt(count);
                        this.odpDao.addReservedNewInfo(entity);
                    }
                }
            ...
            {
                LOGGER.error("addReservedNewInfo error! Exception is {}", e);
            }
    ...
    /**
     * <调用文件上传接口，将用户上传的数据导入hbase>
     * @param fileId  导入文件ID
     * @param rsp 返回消息
     * @throws CException
     * @see [类、类#方法、类#成员]
     */
    private void fileUPloadGW(String fileId, FileUploadRsp rsp)
        throws CException
    {
        FileUploadTableEntity entity = new FileUploadTableEntity();
        String filePath = CommonUtils.getSysConfig("file_local_path");
        ...
        String url =
            "http://" + CommonUtils.getSysConfigValueByKey("bt_servser.opendataplatform.addr")
                + "/rest/dataplatform/uploadFile";
        
        Client client = Client.create();
        WebResource resource = client.resource(url);
        FormDataMultiPart form = new FormDataMultiPart();
        form.field("file", new File(fileName), MediaType.MULTIPART_FORM_DATA_TYPE);
        form.field("fileID", fileId);
        
        String authInfo = CommonUtils.getInterFaceAuthInfo(fileId);
        form.field("authInfo", authInfo);
        
        LOGGER.debug("transfer uploadFile begin. file id is {} and file name id {} and authInfo is {}", new Object[] {
            fileId, fileName, authInfo});
        String result = resource.type(MediaType.MULTIPART_FORM_DATA).post(String.class, form);
        
        LOGGER.debug("transfer uploadFile responese is {}", result);
        UploadFileRsp uploadFileRsp = OdpCommonUtils.parseObject(result, UploadFileRsp.class).createUploadFileRsp();
        if (uploadFileRsp.getResultCode() != 0)
        {
            LOGGER.error("transfer uploadFile error! uploadFileRsp is {}", uploadFileRsp);
            
            rsp.setFile_id(uploadFileRsp.getFileId());
            //文件上传成功
            entity.setFlag(FileDealFlagType.ERROR);
            entity.setFileId(uploadFileRsp.getFileId());
            this.odpDao.insertFileUploadTableInfo(entity);
            
            //生成文件失败，返回上传文件失败异常
            throw new CException(ResultCode.FILE_UPLOAD_INTERFACE_ERROR,
                "transfer to /rest/dataplatform/uploadFile interface error!");
        }
