...
public class TccServiceImpl implements TccService, Serializable
{
     ...
     * 临时停止调度的周期任务hashmap
     */
    private Map<Long, HashMap<String, TaskRunningStateEntity>> tmpStopedTasks =
        ...
        try
        {
            ...
            if (null != taskId && (StringUtils.isNotBlank(cycleId)))
            {
                ...
                while (!processingQueue.isEmpty() && taskRSProcessed.size() < maxCount)
                {
                    ...
                    cycleDeppingRs = getAllCycleDeppingRs(currTask, tasks, curNode.getCycleId());
                    //查询任务周期的状态
        ...
        throws CException
    {
        ...
        if (null == taskRS)
        {
            ...
            LOGGER.error("dependent taskRunningState[taskId={},cycleId={}] do not exist, ignore it.", new Object[] {
                taskId, cycleId});
            throw new CException(ResultCodeConstants.TASK_CYCLE_NOT_EXIST_ERROR, new Object[] {taskId, cycleId});
        }
        ...
        throws Exception
    {
        ...
        {
            LOGGER.error("redo TaskCycles failed, taskRsList is {}", taskRsList, e);
            throw e;
        }
        finally
        {
    ...
    public void scheduleTask()
    {
        ...
        try
        {
            ...
            for (TaskEntity task : taskLst)
            {
                if (TaskState.STOP == task.getTaskState() || tmpStopedTaskEs.containsKey(task.getTaskId()))
                {
                    continue;
                }
                ...
                {
                    //如果是顺序依赖，只需要取一条
                    taskRunStatelst = tccDao.getTopOldNeedRunCycleTasks(taskId, task.getStartTime(), maxCycleId);
                }
    ...
    private void checkLatestStartAlarm(TaskEntity task)
    {
        try
        {
            TaskAlarmThresholdEntity alarmThreshold = tccDao.getAlarmThreshold(task.getTaskId());
            ...
            if (!waittingQueue.isEmpty())
            {
                ...
                while (cycleTaskIter.hasNext())
                {
                    ...
                    if (newRunningResN >= cycleTask.getTaskWeight())
                    {
                        ...
                        LOGGER.info("put cycleTask[taskId=[{}],cycleId=[{}]] into runningTasks.", new Object[] {taskId,
                            cycleId});
    ...
    private void add2WaittingTasks(Long taskId, String cycleId, CycleTask cycleTask)
    {
        ...
        //记录添加到等待队列的元素
        LOGGER.debug("put cycleTask[taskId=[{}],cycleId=[{}]] into waittingQueue[size={}]. ", new Object[] {taskId,
                    ...
                    if (dependRelationLst.get(depTaskId).isFullCycleDepend())
                    {
                        for (String depCycleId : dependCycleIds)
                        {
                            ...
                            if (!priviousDate.before(earliestStartTime))
                            {
                                ...
                                cycleDepR.setDependCycleId(depCycleId);
                                bIgnoreDependError = dependRelationLst.get(depTaskId).isIgnoreError();
     ...
     * 依赖关系是否全部ok
     */
    private boolean isDependsOk(List<CycleDependRelation> cycleDepRLst, Long taskId)
        throws CException
    {
     ...
     */
    private void fillCycle2Current(Long taskId, int cycleType, int cycleLength, String depends)
        throws CException
    {
        //获取最大的cycleID,补齐右边
    ...
    private String getPrevCycleId(Date curTime, int cycleType, int cycleLength, int depCycleType, int depCycleLength)
    {
        ...
        if (compareValue > 0)
        {
            ...
            priviousDate = TccUtil.roll2CycleStartOnBenchDate(curTime, depCycleType, depCycleLength, 0);
        ...
        else
        {
            ...
            priviousDate = TccUtil.roll2CycleStartOnBenchDate(curTime, depCycleType, depCycleLength, -1);
        }
