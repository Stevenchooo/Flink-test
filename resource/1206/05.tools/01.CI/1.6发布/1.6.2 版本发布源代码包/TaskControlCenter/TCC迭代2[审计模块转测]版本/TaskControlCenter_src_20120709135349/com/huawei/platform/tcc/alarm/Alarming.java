    ...
    public void sendAlarm(long taskId, String cycleId, int instanceID, AlarmInfo alarmInfo)
    {
        ...
        try
        {
            ...
            if (null != taskAlarmItems && taskAlarmItems.isEnableAlarm(alarmInfo.getAlarmType()))
            {
                ...
                if (null != toAlarmchannel)
                {
                    alarmFact.setEmailList(toAlarmchannel.getEmailList());
                    alarmFact.setMobileList(toAlarmchannel.getMobileList());
                }
                else if (null != service)
                {
                    alarmFact.setEmailList(service.getAlarmEmailList());
                    alarmFact.setMobileList(service.getAlarmMobileList());
                }
                //保持告警事实
