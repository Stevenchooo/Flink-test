...
public class CycleTask implements Runnable, Comparable<CycleTask>
{
     ...
     * 构造函数
     * @param task 任务
     * @param stepKeyMap 步骤ID与任务步骤的键值对集合
     * @param taskRS 任务运行周期
     * @param cycleDepRLst 周期依赖关系
     */
    public CycleTask(TaskEntity task, HashMap<Integer, TaskStepEntity> stepKeyMap, TaskRunningStateEntity taskRS,
        List<CycleDependRelation> cycleDepRLst)
    {
        this.task = task;
        this.stepKeyMap = stepKeyMap;
        this.taskRS = taskRS;
    ...
    public void run()
    {
        ...
        try
        {
            ...
            //再次判断依赖是否成立
            //addcurrCycleTask2DependTaskList会更新起始依赖列表，
            //但是redo中读取依赖列表的时间点可能刚好发生在当前周期任务启动和调用addcurrCycleTask2DependTaskList之前
            //这将会导致周期任务的redo时，无法控制后续的依赖周期任务能全部停止下来
        ...
        throws CException
    {
            ...
            else
            {
                if (CycleType.HOUR == cycleType || CycleType.DAY == cycleType)
                {
                    ...
                    if (rightEdgeTime.after(task.getRedoEndTime()))
                    {
                        ...
                        //微调1秒
                        rightEdgeTime = tuneSeconds(rightEdgeTime, 1);
                    }
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    //倍率
                    int multiples = (cycleType == CycleType.DAY) ? TccConfig.HOURS_OF_DAY : 1;
                    int integratCycleNum = (task.getRedoDayLength() * TccConfig.HOURS_OF_DAY) / multiples;
                    //获取已经虚拟执行成功的任务周期数
                    Date redoStart = task.getRedoStartTime();
                    Date current = TccUtil.covCycleID2Date(this.getCycleId());
                    long hourNum = (current.getTime() - redoStart.getTime()) / TccConfig.MILLS_PER_HOUR;
                    int cycleNum = (int)(hourNum / multiples) + 1;
                    
                    //如果当前周期要实际执行的话，//保证聚集在一起执行的周期的时间长度刚好小于等于task.getRedoDayLength()天数
                    if (cycleNum % integratCycleNum == 0)
                    {
                        this.execType = ExecuteType.INTEGRATION;
                        int offset = (int)(cycleNum - integratCycleNum);
                        Date starCycleTime =
                            TccUtil.roll2CycleStartOnBenchDate(redoStart, cycleType, task.getCycleLength(), 0);
                        if (starCycleTime.before(redoStart))
                        {
                            offset++;
                        }
                        starCycleTime =
                            TccUtil.roll2CycleStartOnBenchDate(redoStart, cycleType, task.getCycleLength(), offset);
                        this.startCycleId = TccUtil.covDate2CycleID(starCycleTime);
                    }
                    else
                    {
                        Date nextCycleTime =
                            TccUtil.roll2CycleStartOnBenchDate(cycleTime, cycleType, task.getCycleLength(), 1);
                        if (nextCycleTime.before(curDayStart))
                        {
                            //如果不是历史的最后一个周期
                            if (null != task.getRedoEndTime() && !task.getRedoEndTime().before(cycleTime)
                                && nextCycleTime.after(task.getRedoEndTime()))
                            {
                                //如果配置了重做结束时间，
                                this.execType = ExecuteType.INTEGRATION;
                                
                                int offset = (int)(cycleNum - cycleNum % integratCycleNum);
                                Date starCycleTime =
                                    TccUtil.roll2CycleStartOnBenchDate(redoStart, cycleType, task.getCycleLength(), 0);
                                if (starCycleTime.before(redoStart))
                                {
                                ...
                                starCycleTime =
                                    TccUtil.roll2CycleStartOnBenchDate(redoStart,
                                        cycleType,
                                        task.getCycleLength(),
                                        offset);
                                this.startCycleId = TccUtil.covDate2CycleID(starCycleTime);
        ...
        throws CException
    {
        ...
        switch (taskRS.getState())
        {
            case RunningState.INIT:
            {
                ...
                taskRSU.setRunningStartTime(CommonUtils.getCrruentTime());
                tccDao.updateTaskRunningState(taskRSU);
                ...
                case EndBatchFlag.FILESINWAITTING:
                {
                    ...
                    while (CommonUtils.getCrruentTime().before(dateEnd))
                    {
                        ...
                        LOGGER.debug("waittingInput of cycleTask[taskId=[{}],cycleId=[{}]] started. runTimes is [{}],"
                            + "current time is [{}]",
                            new Object[] {getTaskId(), getCycleId(), runTimes, CommonUtils.getCrruentTime()});
                    }
                    break;
                }
        ...
        throws CException
    {
        ...
        int existNum = (null == cycleDepRealationLst) ? 0 : cycleDepRealationLst.size();
        //存在且状态完成的周期任务数
    ...
    private void asyncSendEmail()
    {
        try
        {
            Thread thd = new Thread(new AsyncEmailSender());
            thd.start();
        }
        catch (Exception e)
        {
            LOGGER.error("asyncSendEmail failed!", e);
        }
    }
     ...
     * 异步发送邮件
     * @author  z00190465
     * @version [Internet Business Service Platform SP V100R100, 2012-5-4]
     */
    public class AsyncEmailSender implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                Thread.currentThread().setName(String.format("AsyncEmailSender_%d_%s",
                    taskRS.getTaskId(),
                    taskRS.getCycleId()));
                ...
                {
                    emailsTo = concate2Dist(TccConfig.getEmailsTo(), task.getFailureEmailsTo());
                }
                else if (state == RunningState.TIMEOUT)
                {
                    emailsTo = concate2Dist(TccConfig.getEmailsTo(), task.getFailureEmailsTo());
                }
                else if (state == RunningState.NOBATCH)
                {
                    emailsTo = concate2Dist(TccConfig.getEmailsTo(), task.getFailureEmailsTo());
                }
                ...
                {
                    //如果成功不发送邮件，则直接返回
                    if (null == task.getSuccessSendEmailFlag() || !task.getSuccessSendEmailFlag())
                    {
                        return;
                    }
                    emailsTo = TccConfig.getEmailsTo();
                }
                else
                {
                    return;
                ...
                String subject =
                    TccUtil.getEmailSubject(taskRS.getTaskId(), task.getTaskName(), taskRS.getCycleId(), state);
                String content =
                    TccUtil.getEmailContent(taskRS.getTaskId(), task.getTaskName(), taskRS.getCycleId(), state);
                TccUtil.sendEmail(TccConfig.getEmailFrom(), emailsTo, subject, content);
            }
            catch (Exception e)
            {
                LOGGER.error("Async send Email failed!", e);
            }
        }
        
        /**
         * 拼接邮箱地址，并且防止去除重复的邮箱地址
         */
        private String concate2Dist(String emailsTo, String failureEmailsTo)
        {
            String mEmailsTo = emailsTo;
            if (StringUtils.isBlank(mEmailsTo))
            {
                mEmailsTo = failureEmailsTo;
            }
            else
            {
                if (!StringUtils.isBlank(failureEmailsTo))
                {
                    mEmailsTo += ";" + failureEmailsTo;
                }
            }
            ...
            {
                //去除重复项
                String[] emails = mEmailsTo.split(";");
                List<String> emailList = new ArrayList<String>();
                for (String email : emails)
                {
                    if (!StringUtils.isBlank(email) && !emailList.contains(email.toLowerCase()))
                    {
                ...
                //重新构造emailsTo
                StringBuilder emailsToRtn = new StringBuilder();
                for (String email : emailList)
                {
                    emailsToRtn.append(email);
                    emailsToRtn.append(";");
                }
                
                return emailsToRtn.toString();
            }
            else
            {
                return mEmailsTo;
    ...
    public class StepTimeOutProcessor extends TimerTask
    {
        ...
        public StepTimeOutProcessor(Integer timeoutMinutes, StepRunningStateEntity stepRS)
        {
            ...
            long delay = timeoutMin * TccConfig.MILLS_PER_MINUTES;
