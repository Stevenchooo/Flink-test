...
public class RoleManageAction extends BaseAction
{
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            search.setNames(URLDecoder.decode(roleId, "UTF-8"));
     ...
     * @throws Exception 异常
     */
    public String reqHavePrivileges()
        throws Exception
    {
        HttpServletRequest request =
            (HttpServletRequest)ActionContext.getContext().get(ServletActionContext.HTTP_REQUEST);
        try
        {
            ...
            {
                privileges = new ArrayList<RolePrivilegeInfo>(0);
            }
            else
            {
                Search search = new Search();
                ...
                int page =
                    StringUtils.isEmpty(request.getParameter("page")) ? 0
                        : Integer.valueOf(request.getParameter("page"));
                int rows =
                    StringUtils.isEmpty(request.getParameter("rows")) ? 0
                        : Integer.valueOf(request.getParameter("rows"));
                
                search.setPageIndex((page - 1) * rows);
                search.setPageSize(rows);
                privileges = getOperatorMgnt().getHavePrivileges(search);
                RolePrivilegeInfoEntity rolePrivilegeInfo = new RolePrivilegeInfoEntity();
                rolePrivilegeInfo.setRoleId(roleId);
                count = getOperatorMgnt().getRolePrivilegeCount(rolePrivilegeInfo);
            }
            //JSONObject纯对象
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("total", count);
        ...
        {
            LOGGER.error("reqHavePrivileges fail", e);
            throw e;
        }
    }
        ...
        throws Exception
    {
        ...
        List<String> roleIds = getOperatorMgnt().getRoleIdListNoSystem();
        if (containSystem)
        {
            roleIds.add(0, Role.SYS_ADMIN);
        }
