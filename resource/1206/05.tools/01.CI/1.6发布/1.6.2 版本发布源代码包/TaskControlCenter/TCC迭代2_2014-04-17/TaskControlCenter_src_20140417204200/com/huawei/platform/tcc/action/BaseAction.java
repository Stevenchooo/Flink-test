...
import com.huawei.platform.tcc.utils.Encoder;
...
public class BaseAction extends ActionSupport
{
    ...
    //tcc业务
    private transient TccService tccService;
    
    //portal业务
    private transient TccPortalService tccPortalService;
     ...
     * @param input 输入
     * @return 输入是否合法
     * @throws CException 输入不合法异常
     */
    public String checkInput(String input)
        throws CException
    {
        //无效输入抛出异常
        if (!TccUtil.isValidInput(input))
        {
            LOGGER.error("input is invalid!", TccUtil.noNewLineEncode(input));
            throw new CException(ResultCode.INPUT_INVALID);
        }
        
        return truncatEncode(input);
    }
        ...
        throws PrivilegeNotEnoughException
    {
        if (null != taskList && !taskList.isEmpty())
        {
            ...
            {
                //需要具有重做权限
                if (null == mOperator || !mOperator.canQuery(srcTask.getUserGroup()))
                {
                    param = new PrivilegeNotEnoughParam(srcTask.getUserGroup(), SubPrivilegeType.QUERY);
    ...
    public Operator getOperator()
    {
        ...
        String opName = OperatorMgnt.getOperatorName(request.getSession());
        ...
        throws Exception
    {
        ...
        if (null != taskE)
        {
            if (null == operator || !operator.canModify(taskE.getOsUser()))
            {
                priParams.add(new PrivilegeNotEnoughParam(taskE.getOsUser(), SubPrivilegeType.MODIFY));
            }
        ...
        throws Exception
    {
        ...
        if (null != taskE)
        {
            ...
            {
                priParams.add(new PrivilegeNotEnoughParam(taskE.getUserGroup(), SubPrivilegeType.QUERY));
            }
    ...
    {
        return new Encoder().truncatEncode(text);
    }
