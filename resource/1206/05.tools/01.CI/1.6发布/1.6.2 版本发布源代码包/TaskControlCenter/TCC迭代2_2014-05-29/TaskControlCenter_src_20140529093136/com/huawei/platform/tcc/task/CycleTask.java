...
public class CycleTask implements Runnable, Comparable<CycleTask>
{
    ...
    private static final long EXEC_WAIT = 5000;
        ...
        throws CException
    {
        ...
        if (task.getMultiBatchFlag())
        {
            strict = task.getEndBatchFlag() == EndBatchFlag.NORWAITTING_STRICT;
        }
        
        //如果没有任何批次运行状态
        if (!strict && !tccDao.existsBatchRS(taskRS.getTaskId(), taskRS.getCycleId()))
        {
            this.state = RunningState.NOBATCH;
            taskRSU.setState(RunningState.NOBATCH);
        }
        //如果输入文件个数小于要求的数目
        else if (strict && inputedFiles.size() < task.getInputFileMinCount())
        {
            this.state = RunningState.NOBATCH;
            taskRSU.setState(RunningState.NOBATCH);
        }
        ...
        throws CException
    {
        ...
        switch (task.getEndBatchFlag())
        {
            ...
            case EndBatchFlag.NORWAITTING_STRICT:
            {
                while (CommonUtils.getCrruentTime().before(dateEnd)
                    && inputedFiles.size() < task.getInputFileMinCount())
                {
        ...
        throws CException
    {
        ...
        try
        {
            ...
            while (retryCount < stepSE.getAllowRetryCount())
            {
                ...
                try
                {
                    if (retryCount < stepSE.getAllowRetryCount())
                    {
                        LOGGER.info("sleep {} ms!", retryCount * EXEC_WAIT);
                        Thread.sleep(retryCount * EXEC_WAIT);
                    }
                }
                catch (Exception e)
                {
                    //DO NOTHING
                    LOGGER.debug("stepRS[{}] retry to execute [{}] times,interupt", stepRS, retryCount);
                }
