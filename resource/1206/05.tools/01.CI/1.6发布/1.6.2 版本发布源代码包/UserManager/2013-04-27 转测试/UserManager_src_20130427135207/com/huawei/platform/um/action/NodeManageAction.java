...
public class NodeManageAction extends BaseAction
{
    ...
    {
        //获取所有的节点键值对
        List<NodeInfoEntity> nodes = getUmDao().getNodeInfoList();
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            //返回空数据
            Integer count = umDao.getNodesCount(search);
            //否则以名字模糊查询
            List<NodeInfoEntity> nodes = umDao.getNodes(search);
    ...
    public String saveNode()
    {
        ...
        try
        {
            ...
            if (StringUtils.isNotBlank(rootPwd))
            {
                node.setRootPwd(UMUtil.encrypt(rootPwd));
            }
            ...
            {
                //新增记录
                umDao.addNodeInfo(node);
                
                //通知改变
                Eventor.fireEvent(this, EventType.ADD_NODE, node.getNodeId());
                
                //记录操作日志
                LOGGER.info("add node[{}] successfully!", new Object[] {node});
            ...
            else
            {
                ...
                //更新记录
                umDao.updateNodeInfo(node);
                
                //通知改变
                Eventor.fireEvent(this, EventType.UPDATE_NODE, node.getNodeId());
                
                //记录日志
                LOGGER.info("update node[{}] successfully!", new Object[] {node});
        ...
        catch (Exception e)
        {
            ...
            LOGGER.error("failed to " + opt + " the node[{}].", new Object[] {node, e});
        }
