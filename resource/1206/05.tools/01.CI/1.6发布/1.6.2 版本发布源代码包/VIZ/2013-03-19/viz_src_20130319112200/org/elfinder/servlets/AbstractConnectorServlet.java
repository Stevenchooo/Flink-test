 ...
 * <br>
 * <br>
 * This class is *abstract*, you have to implement prepareConfig() so that the
 * servlet will fit to your needs.<br>
 * This way, a single servlet can use multiple configurations and be used as
 * backend of multiple ElFinder clients.<br>
 * <br>
 * Commands are implemented into the same package, in example:
 * org.elfinder.servlets.commands.MkdirCommand<br>
 * Each command can be extended and overriden for specific needs. All you have
 * to do is to create an new class with the following naming convention:<br>
 * => org.elfinder.servlets.commands.MkdirCommandOverride overrides
 * org.elfinder.servlets.commands.MkdirCommand<br>
 */
public abstract class AbstractConnectorServlet extends HttpServlet {

	private static Logger logger = Logger
			.getLogger(AbstractConnectorServlet.class);
	 ...
	 * A single servlet can manage multiple configurations and be used as
	 * backend of multiple ElFinder clients.
	 * 
	 * @param request
	 * @return
	 * @throws Exception
	 */
	protected abstract AbstractConnectorConfig prepareConfig(
	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			AbstractCommand command = prepareCommand(
					(String) requestParams.get("cmd"), request, response,
					config);
			try {
				command.execute();
			...
			if (!command.isResponseOutputDone()) {
				output(response, command.isResponseTextHtml(), json,
						command.getResponseWriter());
	...
	protected static void output(HttpServletResponse response,
			boolean isResponseTextHtml, JSONObject json,
	 ...
	 * @return
	 */
	protected AbstractCommand prepareCommand(String commandStr,
		...
		if (commandStr != null) {
			...
			// TODO jim: change code
			// String current = (String)
			// request.getParameterMap().get("current");
	...
	protected Class<AbstractCommand> getCommandClassDefault(String commandName) {
		String className = AbstractConnectorServlet.class.getPackage()
				.getName()
				+ ".commands."
