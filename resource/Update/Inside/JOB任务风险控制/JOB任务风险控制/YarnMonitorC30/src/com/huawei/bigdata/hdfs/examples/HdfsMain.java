...
public class HdfsMain {
   ...
   * the path of the krb5.conf and keytab to edit to absolute path in Linux。make
   * the keytab and principal in example to current user's keytab and username
   * 
   */
  ...
  private void write() throws IOException, ParameterException {
    ...
    try {
      HdfsWriter writer = new HdfsWriter(fSystem, DEST_PATH + File.separator
          + FILE_NAME);
      writer.doWrite(in);
  ...
  private void delete() throws IOException {
    ...
    if(fSystem.delete(beDeletedPath,true)){
      System.out.println("success to delete the file " + DEST_PATH
          + File.separator + FILE_NAME);
    }else {
      System.err.println("failed to delete the file " + DEST_PATH
          + File.separator + FILE_NAME);
    }
  }
