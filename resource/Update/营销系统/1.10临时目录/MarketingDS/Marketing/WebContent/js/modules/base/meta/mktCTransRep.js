           ...
           querySubPara : function ($scope, params) {
	          ...
	          if(typeof(cid) == "undefined" || cid == ""){
				...
				return {
					method : 'GET',
					params : params,
					url : this.webRoot + "/api/mktCTransRep/ReportQuery"
				};
			},
...
//				});
				...
				$scope.getMktName = function()
				{
					...
					$("<tr><td width=\"13%\"></td><td colspan=\"4\"><a href=\"javascript:void(0)\" id=\"all\" data-checked=\"false\" >全选</a></td></tr>").appendTo("#ad_tbody");
				...
				$scope.transReportExport = function () {
					...
					var url = $scope.options.meta.webRoot + "/api/mktCTransRep/reportExport?__userKey=" + __userKey;					 
		        ...
		        $scope.mktDetailInfo = function (mktInfoName,adInfoWebName,sid,cid,date)
		        {
		        	for(var i=0;i<$scope.resp.result.length;i++)
		        	{
		        		if($scope.resp.result[i].mktInfoName == mktInfoName
		        		   && $scope.resp.result[i].adInfoWebName == adInfoWebName
		        		   && $scope.resp.result[i].mktLandInfoSID == sid
		        		   && $scope.resp.result[i].reportDate == date
		        		 )
		        		{
		        			if((typeof(cid)!=="undefined" && $scope.resp.result[i].mktLandInfoCID != cid) &&
		        			 (cid !="" && $scope.resp.result[i].mktLandInfoCID != cid))
		        			{
		        				continue;
		        			}
		        			$scope.index= i;
		        	...
		        	dialog({
						title : getLocalTag("id_mkt_info_detail", "详情"),
						ok : true,
						okValue : getLocalTag('confirm', 'Yes'),
						content:document.getElementById('id_mkt_info_detail'),
						lock : true,
						opacity : 0.3
					}).showModal();
