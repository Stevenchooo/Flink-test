	...
	var MetaMktLandHonor = {
			...
			querySubPara : function ($scope, params) {
				...
				return {
					method : 'GET',
					params : params,
					url : this.webRoot + "/api/mktLandHonor/query"
				};
			},
			...
			modifyPara : function ($scope, mktLandInfoAid, pid) {
				return {
					method : 'POST',
					url : this.getWebRoot() + "/api/mktLandHonor/modifyHonor",
					params : {
						pId : mktLandInfoAid,
			...
			modifyDialog : function (mktLandInfoAid, $scope) {
				$scope.callApi({
					url : this.getWebRoot()
					 + "/api/mktLandHonor/querymodifycontents",
					method : "GET",
					params : {
				...
				return 'id_modify_mktLandInfo_honor_dialog';
			},
			...
			extendScope : function ($scope, $http) {
				...
				$scope.modifyAd = function (adInfoId) {
					...
					$scope.callApi({
						url : webRoot + "/api/mktLandHonor/detail",
						method : "GET",
						params : {
							adInfoId : adInfoId
						}
				...
				$scope.landBatchResDownLoad = function()
				{
					...
					var url = $scope.options.meta.webRoot + "/api/mktLandHonor/landBatchResDownLoad?__userKey=" + __userKey + "&fileId=" + fileId;
					location.href = url;
				}
				...
				{
					var url = $scope.options.meta.webRoot + "/api/mktLandHonor/templateDownLoad?__userKey=" + __userKey + "&vmallFlag=false";
					location.href = url;
				};
				...
				$scope.landBatchUpLoad = function(){
					...
					if(0 == flag)
					{
						...
						var url = webRoot + "/api/mktLandHonor/uploadFile?__userKey=" + __userKey +"&vmallFlag=1"+  "&batchFlag=" + batchFlag;
					    $.ajaxFileUpload
					    (
					        {
					            ...
					            success: function (data, status)  
					            {
					            	$("#queryButton").trigger('click');
					            	 ...
					            	 var fail = total -updateSuccess - createSuccess - vmallIgnore - otherIgnore;
					            	...
					            	{
					            	       //tip = "总共处理了" + total + "条记录,成功新增" + createSuccess +"条记录， 成功更新" + updateSuccess +"条记录，忽略vmall平台"+ ignore +"条记录，操作失败" + fail + "条记录";
					            	       
					            	       tip = "总共处理了" + total + "条记录,成功新增" + createSuccess +"条记录， 成功更新" + 
				            	                 updateSuccess +"条记录，忽略vmall平台"+ vmallIgnore +"条记录，"+
				            	                 "忽略第三方平台"+otherIgnore+"条记录，"+"操作失败" + fail + "条记录";
				...
				$scope.batchPublish = function () {
					...
					dialog({
						...
						ok : function () {
							...
							$scope.$apply(function () {
								$scope.callApi({
									url : webRoot + "/api/mktLandHonor/batchPublish",
									method : "POST",
									params : params
								}).success(function (resp, status, headers, config) {
									if (resp.resultCode != 0) {
				...
				$scope.batchLoad = function(){
						...
						$("#dealFlag").val(0);
						dialog({
							title : getLocalTag("honor_landInfo_batchLoad_dialog", "批量录入荣耀官网着陆链接"),
							cancel:true,
							cancelValue : '关闭',
							content : document.getElementById('honor_landInfo_batchLoad_dialog'),
							lock : true,
							opacity : 0.3
						}).showModal();;
						 ...
						 if(typeof(queryMktId) == "undefined" || queryMktId == "" || queryMktId == "-1")
						{
						 ...
						 $.ajax({
								...
								success : function (resp) {
									...
									dialog({
										title : getLocalTag("id_mktLandHonnrEmail_dialog", "选择要通知的人"),
										ok : function () {
											var users = "";
										...
										okValue : "发出邮件",
										content : document.getElementById('id_mktLandHonnrEmail_dialog'),
										lock : true,
										opacity : 0.3
									}).showModal();;
								}
