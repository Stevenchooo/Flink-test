 ...
 * @see  [相关类/方法]
 */
public class EmailAdInfoUserList extends AuthRDBProcessor
{
    //日志
        ...
        throws SQLException
    {
        ...
        //获取抄送邮件列表
        List<String> copySendEmailList = MktUtils.getCopySendEmailList(conn, account, mktinfoId, PageType.AD_PAGE,mainSendEmailList);
        ...
        AdEmailNumInfo numInfo = MktUtils.getAdEmailNum(conn, account, mktinfoId, AdState.FOR_LAND);
        StringBuffer contentBuffer = getAdEmailContentBuffer(numInfo);
        
        String title = "你好，营销活动\"" + mktName + "\"的广告位信息已经录入，请登录营销广告系统并录入着陆链接信息，谢谢，通知人 " + account;
    ...
    private StringBuffer getAdEmailContentBuffer(AdEmailNumInfo numInfo)
    {
        ...
        contentBuffer.append("有" + numInfo.getTotalNum() + "个广告位尚未录入着陆链接，");
        if (numInfo.getVmallNum() > 0)
        {
            contentBuffer.append("其中落地到VMALL广告位" + numInfo.getVmallNum() + "个，");
        }
        ...
        contentBuffer.append("请尽快录入。营销广告分析系统网址: <a href=\"" + StringUtils.getConfigInfo("mktSystemUrl") + "\">"
