...
<#if cr>
  ...
  <form name="createForm" class="css-form" novalidate>
   ...
   <tr>
    <td>${lang.account}:&nbsp;</td>
    <td>
     <input type="text" name="account" id="id_crt_account" ng-model="crt_account" ng-trim="true" ng-maxlength="40" required/>
   ...
   <tr>
    <td>${lang.userRole}:&nbsp;</td>
    <td>
     <select ng-model="crt_role" ng-options="r.role as r.name for r in roles">
     </select>
    </td>
   </tr>
  </table>
...
seajs.use(["modules/base/model", "modules/base/meta/admin"], function(model, MetaAdmin) {
    var meta = new MetaAdmin("${vars.webRoot}", 'admin', ["id","name","createTime","creator","val"]);
    model.showList({
        id:"${id}",
        meta:meta,
