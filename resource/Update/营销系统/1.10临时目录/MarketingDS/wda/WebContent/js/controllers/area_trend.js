...
app.controller('AreaTrendCtrl', ['$scope', '$rootScope', '$stateParams', '$http', '$localStorage','$filter','line', function($scope, $rootScope, $stateParams, $http, $localStorage,$filter,line) {
	...
	//当前省份
	$scope.provinceName = $stateParams.provinceName ? $stateParams.provinceName : '';
	...
	//查询汇总数据
	fetchVisitorTrendSummary();
	
	function fetchVisitorTrendSummary() {
		
		$http({
			method: 'GET',
			url: __urlRoot + '/wda/api/wda/visitor/area/trend/summary',
			params: {
				__userKey : __userKey,
				site_id : $scope.sid,				
				periodTime  : getPeriodTime(),			
				huanBiDate  : getPeriodTime(true),
				province_name: encodeURI($scope.provinceName)
			}
	...
	fetchVisitorTrendKpiData();
	
	/*
	 * 查询kpi值
	 */
	function fetchVisitorTrendKpiData() {
    	$http({
			method: 'GET',
			url: __urlRoot + '/wda/api/wda/visitor/trend/kpi',
			params: {
				__userKey : __userKey,
				site_id : $scope.sid,
				periodTime : getPeriodTime(),
		    ...
		    if(resp.resultCode == 0) {
				line.createChart('areaTrendKpi', resp);
		    }
		})
		  .error(function(resp, status, headers, config) {
			  $scope.sumHttp ++;
    ...
    fetchVisitorTrendList();
    function fetchVisitorTrendList() {
    	
    	$http({
			method: 'GET',
			url: __urlRoot + '/wda/api/wda/visitor/trend/list',
			params: {
				__userKey : __userKey,
				site_id : $scope.sid,
				periodTime  : getPeriodTime(),			
		    ...
		    if(resp.resultCode == 0) {		
		    	$scope.websitesJson.curpage = 1;
		        $scope.visitor_trend_list_data = resp.data;
		        $scope.selectDate = resp.selectDate;
		        $scope.visitor_trend_list_hbdata = resp.hbdata;
	...
	$scope.timePickerChange = function(){
		...
		if ($scope.separatedDays) { // 如果前后时间不同，就按天展示
			$scope.time_dim_value = 2;
		} else {
			$scope.time_dim_value = 1;
		}
