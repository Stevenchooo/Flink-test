app.controller('ReferrerDirectCtrl', [ '$scope', '$rootScope', '$http', '$filter','$stateParams', '$localStorage','lineRef', function($scope, $rootScope, $http, $filter, $stateParams, $localStorage,lineRef) {
	
	//设置当前选择的tab页
	$scope.modifyParentScope(4);
	...
	$scope.setCurPage = function(page) {
		if (page == $scope.referDirectJson.curpage) {
			return "pgcur";
		} else {
			return "";
		}
	...
	$scope.selectPage = function(page) {
		if (page <= $scope.referDirectJson.totalPage && page >= 1) {
			$scope.referDirectJson.curpage = page;
		}
	};
    ...
    function fetchKpiLineData() {
    	...
    	$http({
		    ...
		    if(resp.resultCode == 0) {
				lineRef.createChart('refferDirectLine', resp, true);
		    }
		})
		  .error(function(resp, status, headers, config) {
			  $scope.sumHttp ++;
    ...
    function fetchPromotionReferrerList() {
    	...
    	$http({
		    ...
		    if(resp.resultCode == 0) {
		    	$scope.referDirectJson.curpage = 1;
		    	$scope.referDirectJson.data =  resp.data;
