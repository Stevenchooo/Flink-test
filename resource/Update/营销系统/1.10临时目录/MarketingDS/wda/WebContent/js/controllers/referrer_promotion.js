app.controller('PromotionReferrerCtrl', [ '$scope', '$rootScope', '$http', '$filter','$stateParams', '$localStorage','lineRef','pie', function($scope, $rootScope, $http, $filter, $stateParams, $localStorage,lineRef,pie) {
	//设置当前选择的tab页
	$scope.modifyParentScope(2);
	...
	$scope.setCurPage = function(page) {
		if (page == $scope.promotionReferJson.curpage) {
			return "pgcur";
		} else {
			return "";
		}
	...
	$scope.selectPage = function(page) {
		if (page <= $scope.promotionReferJson.totalPage && page >= 1) {
			$scope.promotionReferJson.curpage = page;
		}
	};
	...
	function fetchKpiPieData() {
    	...
    	$http({
		    ...
		    if(resp.resultCode == 0) {
		        ...
		        pie.createChart('refferProPie', pieData,'');
		    }
		})
		  .error(function(resp, status, headers, config) {
			  $scope.sumHttp ++;
    ...
    function fetchKpiLineData() {
    	...
    	$http({
		    ...
		    if(resp.resultCode == 0) {
				lineRef.createChart('refferProLine', resp, true);
		    }
		})
		  .error(function(resp, status, headers, config) {
			  $scope.sumHttp ++;
    ...
    function fetchPromotionReferrerList() {
    	...
    	$http({
		    ...
		    if(resp.resultCode == 0) {
		    	$scope.promotionReferJson.curpage = 1;
		    	$scope.promotionReferJson.data = resp.data;
