app.controller('ReferrerTrend2Ctrl', ['$scope', '$rootScope', '$filter', '$stateParams', '$http', '$localStorage','line', function($scope, $rootScope, $filter, $stateParams, $http, $localStorage,line) {
	...
	//当前访问来源
	$scope.refName = $stateParams.refName ? $stateParams.refName : '';
	...
	function fetchReferrerTrendSummary() {
		$http({
			method: 'GET',
			url: __urlRoot + '/wda/api/wda/website/referrer/trend2/summary',
			params: {
				__userKey : __userKey,
				site_id : $scope.sid,				
				periodTime  : getPeriodTime(),			
				huanBiDate  : getPeriodTime(true),
				ref_name:  encodeURI($scope.refName)
			}
	...
	function fetchReferrerTrendKpiData() {
    	$http({
			method: 'GET',
			url: __urlRoot + '/wda/api/wda/website/referrer/trend2/kpi',
			params: {
				__userKey : __userKey,
				site_id : $scope.sid,
				periodTime : getPeriodTime(),
		    ...
		    if(resp.resultCode == 0) {
				line.createChart('refferTrend2Line', resp);
		    }
		})
		  .error(function(resp, status, headers, config) {
			  $scope.sumHttp ++;
    ...
    function fetchReferrerTrendList() {
    	...
    	$http({
			method: 'GET',
			url: __urlRoot + '/wda/api/wda/website/referrer/trend2/list',
			params: {
				__userKey : __userKey,
				site_id : $scope.sid,
				periodTime  : getPeriodTime(),			
