app.controller('WebsiteStatisticsLatest30Ctrl', ['$scope', '$rootScope', '$filter', '$http', '$stateParams', '$localStorage','line', function($scope, $rootScope, $filter, $http, $stateParams, $localStorage,line) {
		...
		var theMonDate = moment(moment().subtract(29,"days").format(timeFormatter));
		...
		$scope.setCurPage = function(page) {
			if (page == $scope.websitesThirtyJson.curpage) {
				return "pgcur";
			} else {
				return "";
			}
		...
		$scope.selectPage = function(page) {
			if (page <= $scope.websitesThirtyJson.totalPage && page >= 1) {
				$scope.websitesThirtyJson.curpage = page;
			}
		};
		...
		function fetchKpiData() {
	    	...
	    	$http({
			    ...
			    if(resp.resultCode == 0) {
					line.createChart('latest30Kpi', resp);
			    }
			})
			  .error(function(resp, status, headers, config) {
				  $scope.sumHttp ++;
	    ...
	    function fetchWebsiteTrendList() {
	    	...
	    	$http({
			    ...
			    if(resp.resultCode == 0) {
			    	$scope.websitesThirtyJson.curpage = 1;
			        $scope.website_trend_list_data = resp.data;		
			        $scope.websitesThirtyJson.data = resp.data;
			    }
			  })
			  .error(function(resp, status, headers, config) {
				  $scope.sumHttp ++;
