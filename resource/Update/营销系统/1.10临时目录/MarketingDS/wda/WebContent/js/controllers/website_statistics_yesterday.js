app.controller('WebsiteStatisticsYesterdayCtrl', ['$scope', '$rootScope', '$filter', '$http', '$stateParams', '$localStorage', 'line', function($scope, $rootScope, $filter, $http, $stateParams, $localStorage,line) {
	...
	var timeDim = {
		...
		$scope.setCurPage = function(page) {
			if (page == $scope.websitesStaticYesJson.curpage) {
				return "pgcur";
			} else {
				return "";
			}
		...
		$scope.selectPage = function(page) {
			if (page <= $scope.websitesStaticYesJson.totalPage && page >= 1) {
				$scope.websitesStaticYesJson.curpage = page;
			}
		};
		...
		$scope.allOptions = 
			[ { key : '浏览量(PV)', value: 'pv', checked: true}, 
			  ...
			  { key : '新访客比率', value: 'nuv_rate', checked: false}, 
			  { key : 'IP数', value: 'ip', checked: false}, 
			  { key : '平均访问时长', value: 'avg_visit_time',checked: false}, 
			  { key : '平均访问页面', value: 'avg_visit_pages', checked: false} 
		...
		var getPeriodByDatePick = function() {
			var startDiff = $scope.datePicker.date.startDate.diff(theCurDate, "days");
		...
		function fetchKpiData() {
	    	...
	    	$http({
			    ...
			    if(resp.resultCode == 0) {
					line.createChart('yesterdayKpi', resp);
			    }
			})
			  .error(function(resp, status, headers, config) {
				  $scope.sumHttp ++;
	    ...
	    function fetchWebsiteTrendList() {
	    	$http({
			    ...
			    if(resp.resultCode == 0) {
			    	$scope.websitesStaticYesJson.curpage = 1;
			    	$scope.websitesStaticYesJson.data= resp.data;		  
			    }
			  })
			  .error(function(resp, status, headers, config) {
				  $scope.sumHttp ++;
		...
		$scope.huanbi = function() {
			...
			if($scope.isHuanBi) {
				...
				$scope.datePicker.date1.endDate = $scope.datePicker.date.startDate.clone().subtract(1, "days");
		        $scope.datePicker.date1.startDate = $scope.datePicker.date1.endDate.clone().subtract($scope.separatedDays, "days");
			} else {
				$scope.canSelectedKpi = 2;
