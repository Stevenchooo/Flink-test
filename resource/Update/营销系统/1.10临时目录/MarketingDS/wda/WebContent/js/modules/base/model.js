define("modules/base/model", ['jquery', "modules/base/tree", "dialog", "cookie", 'jquery/dialog/4.1.7/skins/default.css'],
	function(require, exports, module) {
    var $ = require("jquery");
    var tree = require("modules/base/tree");
    var mListOpts = {};

	exports.showList = function(opts) {
	    require("dialog");
	    ...
	    cookie.set(__cookieHeader + "scopeId", opts.id, {path:_path, expires:1, secure:false});
	    ...
	    app.controller("ListCtrl", function($scope, $http) {
	    	...
	    	$scope.calcPage=function(resp) {
		    	...
		    	if(pageNum > 1) {
			    	if(curPage > 0) {
			    		pages.push({from:(curPage-1)*opts.perPage, cls:'button forePage', str:''});
			    	}
			    	...
			    	for(; i < end; i++) {
			    		if(i != curPage) {
			    			...
			    			pages.push({from:from, cls:'cur_page', str:(i+1)});
			    		}
			    		from += opts.perPage;
			    	}
			    	if(pageNum > maxPgNum) {
			    	...
			    	if(pageNum > 1 && curPage < pageNum - 1) {
			    		pages.push({from:(curPage+1)*opts.perPage, cls:'button nextPage', str:''});
			    	}
		    	}
	    	...
	    	$scope.refresh=function() {
        		...
        		$scope.refreshTree($scope.options.meta.getName());
	    	};
	    ...
	    angular.bootstrap(document.body, ['ModelList']);
	};
