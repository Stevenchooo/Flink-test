delimiter //
DROP PROCEDURE IF EXISTS sp_visitor_area_userinsight_age;
CREATE PROCEDURE sp_visitor_area_userinsight_age
(
	OUT pRetCode    INT,
...
proc: BEGIN
	...
	set vSelectSql = concat('select dim_value as y,cookies as x from dw_cooperation_wda_result_dm where 1=1');
      ...
      set vSelectSql = concat(vSelectSql,'and pt_dim = ''age'' group by y order by y');
      SET @Sql = vSelectSql;
    ...
    set vSelectSql = concat('select  dim_value as y, SUM(cookies) as x from dw_cooperation_wda_result_dm where 1=1');
    if(pFromDate is not null and pToDate is not null) then
        set vSelectSql = concat(vSelectSql, ' and pt_d >= ', DATE_FORMAT(pFromDate, '%Y%m%d'), ' and pt_d <= ', DATE_FORMAT(pToDate, '%Y%m%d'));  
      end if;
      if(pSiteId is not null and '' != pSiteId) then
