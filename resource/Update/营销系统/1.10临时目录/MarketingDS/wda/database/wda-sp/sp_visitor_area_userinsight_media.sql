delimiter //
DROP PROCEDURE IF EXISTS sp_visitor_area_userinsight_media;
CREATE PROCEDURE sp_visitor_area_userinsight_media
(
	OUT pRetCode    INT,
...
proc: BEGIN
	...
	set vSelectSql = concat('select dim_value as x, cookies as y from dw_cooperation_wda_result_dm where 1=1');
      ...
      set vSelectSql = concat(vSelectSql,'and pt_dim = ''media'' group by x order by y desc');
      SET @Sql = vSelectSql;
