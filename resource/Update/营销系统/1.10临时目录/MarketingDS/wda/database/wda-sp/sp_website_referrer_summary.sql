delimiter //
DROP PROCEDURE IF EXISTS sp_website_referrer_summary;
CREATE PROCEDURE sp_website_referrer_summary
(
    OUT pRetCode    INT,    
...
proc: BEGIN
    ...
    if(pFromDate = pToDate and pFromDate = curdate()) then
        ...
        set vSelectSql = concat('select SUM(pv) as pv, SUM(visits) as visits, round(SUM(total_visit_time) / SUM(visits), 2) as avg_visit_time, round(SUM(total_visit_pages) / SUM(visits), 2) as avg_visit_pages from dw_wda_websites_summary_by_referrer_hm where 1=1');
    ...
    set vSelectSql = concat('select SUM(pv) as pv, SUM(visits) as visits, round(SUM(total_visit_time) / SUM(visits), 2) as avg_visit_time, round(SUM(total_visit_pages) / SUM(visits), 2) as avg_visit_pages from dw_wda_websites_summary_by_referrer_dm where 1=1');
