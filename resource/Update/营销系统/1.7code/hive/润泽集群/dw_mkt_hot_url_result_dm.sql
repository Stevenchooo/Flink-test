...
use biwarehouse;
CREATE EXTERNAL TABLE IF NOT EXISTS dw_mkt_hot_url_result_dm(
     activtiy_id    STRING,
     sid            STRING,
     cid            STRING,
     url            STRING,
...
LOCATION '$hadoopuser/data/DW/dw_mkt_hot_url_result_dm';

INSERT OVERWRITE TABLE dw_mkt_hot_url_result_dm
PARTITION (pt_d = '$date')
select 
    t.activtiy_id,
    t.sid,
...
(
    ...
    (
        ...
        (  
            ...
            where bounces=1 and !IsEmptyUDF(sid)
            group by sid,url
        ) t
